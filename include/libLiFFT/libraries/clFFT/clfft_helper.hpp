...
namespace clFFT
{
    ...
    template<typename T>
    inline void check_error(T err, const char* func, const char *file,
                            const int line)
    {
        if(CL_SUCCESS != err) {
            throw std::runtime_error(
                    "OpenCL error " + std::string(getOpenCLErrorString(err)) + " ["
                            + std::to_string(err) + "]" + " " + std::string(file)
    ...
    inline cl_ulong getMaxGlobalMemSize(cl_device_id dev_id)
    {
        cl_ulong value = 0;
        // print device name
        CHECK_CL(clGetDeviceInfo(dev_id, CL_DEVICE_GLOBAL_MEM_SIZE, sizeof(cl_ulong),
                                 &value, NULL));
        return value;
    }
    ...
    inline cl_device_type getDeviceType(cl_device_id dev_id)
    {
        ...
        CHECK_CL(clGetDeviceInfo(dev_id, CL_DEVICE_TYPE, sizeof(cl_device_type),
                                 &dev_type, NULL));
        return dev_type;
    }
    ...
    inline std::string listClDevices()
    {
        ...
        for(cl_uint i = 0; i < num_of_platforms; i++) {
            CHECK_CL(clGetDeviceIDs(platform_ids[i], CL_DEVICE_TYPE_ALL, 0, NULL,
                                    &num_of_devices));
            cl_device_id* devices = new cl_device_id[num_of_devices];
            if(clGetDeviceIDs(platform_ids[i], CL_DEVICE_TYPE_ALL, num_of_devices,
                                         ...
                                         cl_uint id_platform, cl_uint id_device)
    {
        ...
        cl_uint num_of_devices = 0;
        CHECK_CL(clGetDeviceIDs(platform_ids[id_platform], CL_DEVICE_TYPE_ALL, 0,
                                NULL, &num_of_devices));
        ...
        CHECK_CL(clGetDeviceIDs(platform_ids[id_platform], CL_DEVICE_TYPE_ALL,
